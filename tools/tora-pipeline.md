...
5. **Übersetzen & ausrichten (je Kapitel ein Opus-Subagent):** Brief nach den KRITERIEN unten; Format-/Stilreferenz `tora/daten/work/dewarim-kap2.json`. Der Agent schreibt die Arbeitsdatei `tora/daten/work/<slug>-kap<N>.json` mit `he`-Chunks (Quellreihenfolge) und `de`-Chunks (natürliche deutsche Reihenfolge, verknüpft über gemeinsame `id`).
